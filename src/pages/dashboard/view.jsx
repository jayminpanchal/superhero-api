...
const Dashboard = ({
  ...
  const sendQuery = (q) => {
    searchSuperHerosWatcher({ name: q });
  };

  const renderSuperHeros = () => {
    if (superheros.length === 0) return <EmptySearch />;
    return superheros.map((item, index) => (
      <SuperHero
        superhero={item}
        key={`SUPER_HERO_${index}`}
  ...
  return (
    <div className="container">
      ...
      <div className="content-container">{renderSuperHeros()}</div>
    </div>
  );
};
